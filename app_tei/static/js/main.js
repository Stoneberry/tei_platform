$(document).ready(
	function() {
		loadDocumentTitles();
		let condition1 = (window.location.pathname == '/tei_viewer/corpus');
		if (condition1) {
			getFormItems();
	}
});
...
function catchSearchQuery() {
	...
	var cur_api_url = "/tei_viewer/api/documents?" + query_values;
...
function insertDocument(doc_id, doc_info) {
  ...
  <path d="M3 5.5a.5.5 0 0 1 .5-.5h9a.5.5 0 0 1 0 1h-9a.5.5 0 0 1-.5-.5zM3 8a.5.5 0 0 1 .5-.5h9a.5.5 0 0 1 0 1h-9A.5.5 0 0 1 3 8zm0 2.5a.5.5 0 0 1 .5-.5h6a.5.5 0 0 1 0 1h-6a.5.5 0 0 1-.5-.5z"/>
	...
	if (doc_info["xml_format"] === 1) {
		constr1 += `<span class="badge bg-danger rounded-pill">xml</span>`;
	}
	if (doc_info["json_format"] === 1) {
		constr1 += `<span class="badge bg-success rounded-pill">json</span>`;
	}
...
function loadDocumentTitles() {
	...
	$.get("/tei_viewer/api/documents", function(data) {
